    ...
    <form action="{{ route('admin.promociones.store') }}" method="POST">
        ...
        <div class="flex items-center mb-4">
            ...
            <input type="text" id="tipo" name="tipo" value="{{ old('tipo') }}"
                class="flex-1 border border-yellow-500 px-3 py-2 rounded-md focus:outline-none focus:ring focus:ring-yellow-400"
                placeholder="Ejemplo: Porcentaje, Monto, 2x1, etc." required>
        </div>
        ...
        <div class="flex items-center mb-6">
            ...
            <div class="flex-1">
                <label class="inline-flex items-center mr-4">
                    <input type="radio" name="estado" value="ACTIVO" class="mr-1" {{ old('estado', 'ACTIVO') == 'ACTIVO' ? 'checked' : '' }}>
                    Activo
                </label>
                <label class="inline-flex items-center">
                    <input type="radio" name="estado" value="INACTIVO" class="mr-1" {{ old('estado') == 'INACTIVO' ? 'checked' : '' }}>
                    Inactivo
                </label>
            </div>
        </div>
        ...
        <div class="flex justify-between">
            <button type="submit" class="bg-blue-600 text-black px-6 py-2 rounded font-semibold hover:bg-blue-500">Guardar</button>
            <a href="{{ route('admin.promociones.index') }}" class="bg-yellow-600 text-white px-4 py-2 rounded hover:bg-yellow-500">Cancelar</a>
        </div>
    </form>
